...
fn parametric_surfaces_roundtrip() {
    let surfaces: Vec<(&str, Box<dyn ParametricSurface>)> = vec![
        ...
        (
            "parabolical",
            Box::new(xrt_rs::oes::surfaces::parabolical::ParabolicalSurface::new(
                10.0, 0.0,
            )),
        ),
        (
            "hyperbolic",
            Box::new(xrt_rs::oes::surfaces::hyperbolic::HyperbolicSurface::new(
                100.0, 50.0, 0.0,
            )),
        ),
    ];
...
fn grating_deflection_changes_direction() {
    use xrt_rs::core::beam::{Beam, RayState};
    use xrt_rs::oes::aperture::Aperture;
    use xrt_rs::oes::reflect::{DeflectionMode, reflect_local};
    ...
    let results = reflect_local(
        ...
        None,
        None,
        &xrt_rs::math::rootfind::RootFindConfig::default(),
    );
