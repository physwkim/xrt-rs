...
use xrt_rs::materials::crystal::CrystalGeometry;
use xrt_rs::materials::crystal_variants::{CrystalDiamond, CrystalSi};
use xrt_rs::materials::data::ScatteringTable;

fn load_fixture(name: &str) -> serde_json::Value {
    let path = format!("{}/validation/fixtures/{name}", env!("CARGO_MANIFEST_DIR"));
...
fn test_darwin_width(fixture_name: &str, hkl: [i32; 3]) {
    ...
    let dw_s = si
        .base
        .get_darwin_width(&e_arr, b, xrt_rs::materials::crystal::Polarization::S, &si)
        .unwrap();

    // P-polarization
    let dw_p = si
        .base
        .get_darwin_width(&e_arr, b, xrt_rs::materials::crystal::Polarization::P, &si)
        .unwrap();
    ...
    let dw_s_high = si
        .base
        .get_darwin_width(&e_high, b, xrt_rs::materials::crystal::Polarization::S, &si)
        .unwrap();
    assert!(
        dw_s_high[0] < dw_s[0],
...
fn golden_debye_waller_effect() {
    ...
    let stol = array![theta_b[0].sin() / (xrt_rs::core::consts::CH / 10000.0)];
...
fn golden_mosaicity_broadens_darwin_width() {
    ...
    let dw_no = si_no_mos
        ...
        .get_darwin_width(
            &e_arr,
            -1.0,
            xrt_rs::materials::crystal::Polarization::S,
            &si_no_mos,
        )
        .unwrap();
    let dw_mos = si_mos
    ...
    for (fixture_name, hkl) in [
        ...
        let dw_p = si
            ...
            .get_darwin_width(
                &e_arr,
                -1.0,
                xrt_rs::materials::crystal::Polarization::P,
                &si,
            )
            .unwrap();
