...
use xrt_core::consts::AVOGADRO;
use xrt_core::error::XrtError;
use xrt_math::f0::f0_scalar;

use crate::crystal::{CrystalBase, CrystalGeometry, StructureFactor};
    ...
    pub fn new(
        ...
        for (&atom_name, &frac) in atoms.iter().zip(fractions.iter()) {
            let elem = if let Ok(z) = atom_name.parse::<usize>() {
                ...
                let z = crate::data::element_z(atom_name)
